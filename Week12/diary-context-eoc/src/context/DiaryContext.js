  ...
  switch (action.type) {
    ...
    case 'edit_post':
      return state.map((post) => {
        // if the id matches the one we want to edut
...
export const DiaryProvider = ({children}) => {
  ...
  const [state, dispatch] = useReducer(postReducer, [{id: 123, title:"fake post 1", content: "here is some content we don't have to type everytime"}])
